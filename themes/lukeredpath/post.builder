...
xml.feed :'xml:lang' => 'en-US', :xmlns => 'http://www.w3.org/2005/Atom' do
  ...
  xml.link :type => 'text/html', :href => absolute_url("/#{@post.id}.html"), :rel => 'alternate'
  xml.link :type => 'application/atom+xml', :href => absolute_url("/#{@post.id}/feed"), :rel => 'self'
  xml.title "Comments on „#{@post.title}“ #{marley_config.blog.name}"
  ...
  @post.comments.each_with_index do |comment, index|
    xml.entry do |entry|
      ...
      entry.link :type => 'text/html', :href => absolute_url("/#{@post.id}.html#comment_#{index}"), :rel => 'alternate'
