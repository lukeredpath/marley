...
GitHubBadge.Launcher = new function() {
  ...
  this.loadedLibraries = function() {
    ...
    if (is_black == 'black' || jQuery.color.almostBlack(jQuery('#github-badge').parent().css('background-color'))) {
      requestStylesheet(GitHubBadge.Launcher.basePath() + 'ext/stylesheets/black_badge.css', 'black_badge');
    } else {
      requestStylesheet(GitHubBadge.Launcher.basePath() + 'ext/stylesheets/badge.css', 'badge');
    }
...
GitHubBadge.Launcher.requestContent = function( url, callback ) {
  ...
  if ("jQuery" in window) {
    jQuery.getScript(url, eval(callback));
  } else {
